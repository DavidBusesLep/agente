  ...
  try {
    switch (extension) {
      ...
      case 'pdf':
        ...
        try {
          // Función interna para intentar OCR (pdftoppm + tesseract)
          const doPdfOcr = async (): Promise<{ text: string; metadata: any } | null> => {
            try {
              ...
              try {
                console.log('🔎 Intentando OCR: conversión PDF→PNG con pdftoppm');
                ...
                console.warn('💡 Instala Poppler y agrega pdftoppm al PATH.');
          ...
          // OCR forzado por entorno: saltear parser nativo
          const forceOcrEnv = String(process.env.FORCE_PDF_OCR || '').toLowerCase();
          const forceOcr = forceOcrEnv === '1' || forceOcrEnv === 'true' || forceOcrEnv === 'yes';
          if (forceOcr) {
            console.log('⛓️  OCR forzado por FORCE_PDF_OCR');
            const ocrResult = await doPdfOcr();
            if (ocrResult) return ocrResult;
            console.log('⚠️ OCR forzado falló; continuando con parser nativo');
          }
          ...
          if (ocrEnabled) {
            try {
              console.log('🔎 Intentando OCR (post-fallback)...');
              const ocrResult = await doPdfOcr();
              if (ocrResult) return ocrResult;
            } catch (ocrSetupErr: any) {
              console.warn('⚠️ OCR no se pudo ejecutar:', ocrSetupErr?.message || ocrSetupErr);
